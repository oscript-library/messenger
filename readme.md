...
Для использования необходимо создать входящий вебхук в вашем Битрикс с правом `Чаты и уведомления (im)` и получить его адрес.

#### Пример Битрикс

	Мессенджер = Новый Мессенджер();
	Мессенджер.ИнициализироватьТранспорт("Битрикс", Новый Структура("ВебХук", ВебХук));

    ChatID = "123";
    Сообщение = "[B]Тест[/B][BR][I]" + ТекущаяДата() + "[/I]";

    ДополнительныеПараметрыСообщения = Новый Структура();
    ДополнительныеПараметрыСообщения.Вставить("SYSTEM", "Y");
    ДополнительныеПараметрыСообщения.Вставить("REPLY_ID", 234683);

    ОтветHTTP = Мессенджер.ОтправитьСообщение("Битрикс", ChatID, Сообщение, , , ДополнительныеПараметрыСообщения);

### Макс

Для использования необходимо создать чат-бота и получить его токен. [https://dev.max.ru/docs](https://dev.max.ru/docs)

#### Пример Макс

```bsl
Мессенджер = Новый Мессенджер();
Мессенджер.ИнициализироватьТранспорт("Макс", Новый Структура("Токен", "..."));

Мессенджер.ОтправитьСообщение("Макс", "Чат:381931907", "Тестовый привет чату");
Мессенджер.ОтправитьСообщение("Макс", "75448381", "Тестовый привет пользователю");
Мессенджер.ОтправитьСообщение("Макс", "Пользователь:75448381", "Тестовый привет-2 пользователю");
```
