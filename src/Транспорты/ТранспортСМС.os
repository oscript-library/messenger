...
Процедура Инициализация(ПараметрыИнициализации) Экспорт
	...
	ИначеЕсли КодОператора = ДоступныеОператоры().smsclub Тогда

		Заголовки = Новый Соответствие;
		Заголовки.Вставить("Authorization", "Bearer " + ПараметрыИнициализации.Токен);
		Заголовки.Вставить("Content-Type", "application/json");
		
		ПараметрыАвторизации = Новый Структура("ШаблонТелаЗапроса, Логин, Пароль, Подпись", ПолучитьШаблонТелаЗапросаSmsClub(), Логин, Пароль, Подпись);
		ПараметрыАвторизации.Вставить("ИмяСервера", "https://im.smsclub.mobi");
		ПараметрыАвторизации.Вставить("URL", "sms/send");
		ПараметрыАвторизации.Вставить("Заголовки", Заголовки);
...
Функция ДоступныеОператоры()
	...
	Если ДоступныеОператорыSMS = Неопределено Тогда
		...
		ДоступныеОператорыSMS = Новый Структура("smsbliss, infobip, sms4b, smsclub", "smsbliss", "infobip", "sms4b", "smsclub");
...
Функция ПолучитьШаблонТелаЗапросаSmsClub ()
